...
- **`upload-screenshot.sh` 404 SHA bug** — when the screenshot file doesn't exist on the `pr-screenshots` branch, `gh api ... --jq '.sha'` outputs the full 404 JSON error body instead of empty string (the `|| true` catches the exit code but not the output). This corrupts the upload JSON payload, causing a 400 error. **Use the `/pr-screenshot` skill instead**, which builds the JSON correctly with `jq -n --rawfile`.
