...
Custom sub-agent definitions live in `.claude/agents/<name>.md`. Sub-agents run in a **fresh context window** via the Task tool, with no access to the parent conversation's history. **Agent definition files are NOT automatically loaded** — the prompt must explicitly tell the agent to read its definition file (e.g. `Read /path/to/.claude/agents/pr-reviewer.md`).

**`pr-reviewer`** — Reviews a single PR diff for correctness, safety, documentation accuracy, and adherence to project conventions. Enforces a strict zero-issues-before-approval bar: wording nits, misleading documentation, inconsistent terminology, and minor style issues all block approval. The reviewer also reads PR comments for dispute context — if a previous review item was disputed with evidence (e.g. a link to official API docs), the reviewer accepts the correction and does not re-raise it. For claims about external API behavior, the reviewer must verify against official docs before flagging as incorrect. After evaluation, the sub-agent **submits the review itself** via the GitHub Actions bot (`gh workflow run "Approve PR"` or `"Request Changes"`), then reports back with a structured response: first line is `APPROVE` or `REQUEST_CHANGES`, followed by reasoning (numbered issue list for rejections) and confirmation that the workflow was triggered. The prompt must tell the agent to read `.claude/agents/pr-reviewer.md` for its full instructions. Used by the `/merge-prs` skill to ensure each PR is reviewed independently (not influenced by the conversation that created the code).

**Disputing a review** — if the reviewer requests changes based on an incorrect factual claim (e.g. misunderstanding how an external API works), add a comment to the PR with evidence (link to official docs, quote the relevant text). On the next review cycle, the reviewer reads PR comments and will accept valid disputes. This avoids infinite loops where the reviewer keeps re-raising an incorrect objection.
