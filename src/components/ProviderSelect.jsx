    ...
    <div className="flex flex-col gap-2">
      <div className="flex flex-col gap-1">
        ...
        <Select value={providerId} onValueChange={onProviderChange} disabled={disabled}>
          <SelectTrigger className="w-full font-ocr text-xs bg-white/[0.04] border-white/10 text-card-foreground">
            <SelectValue />
          </SelectTrigger>
          <SelectContent className="bg-[#1a1e26] border-white/10">
            {providerList.map(p => (
              <SelectItem key={p.id} value={p.id} className="font-ocr text-xs">
                {p.name}
              </SelectItem>
            ))}
          </SelectContent>
      ...
      <div className="flex flex-col gap-1">
        ...
        <Select value={modelId} onValueChange={onModelChange} disabled={disabled}>
          ...
          <SelectContent className="bg-[#1a1e26] border-white/10">
            {models.map(m => (
              <SelectItem key={m.id} value={m.id} className="font-ocr text-xs">
                {m.name}
              </SelectItem>
            ))}
          </SelectContent>
